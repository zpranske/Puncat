...
function[S] = processSummary(summary_files)
    ...
    summary_format = [{'Slice'} {'Count'} {'TotalArea'} {'AverageSize'}...
        {'x_Area'} {'Mean'} {'Perim_'} {'Feret'} {'FeretX'} {'FeretY'}...
        {'FeretAngle'} {'MinFeret'} {'IntDen'}];
    
    S = [];
     for i = 3:length(summary_files)
        temp_S = readtable([summary_files(i).folder '\' summary_files(i).name]);
        if~(temp_S.Properties.VariableNames == summary_format)
            error(['Header format for file ' summary_files(i).name ' is not correct!'])
        end;
     ...
     %Rename variables to prevent duplicates when measure and summary tables are merged
     S = renamevars(S,'Mean','Mean_S');
     S = renamevars(S,'Perim_','Perim_S');
     S = renamevars(S,'Feret','Feret_S');
     S = renamevars(S,'FeretX','FeretX_S');
     S = renamevars(S,'FeretY','FeretY_S');
     S = renamevars(S,'FeretAngle','FeretAngle_S');
     S = renamevars(S,'MinFeret','MinFeret_S');
     S = renamevars(S,'IntDen','IntDen_S');
     S = renamevars(S,'x_Area','PercentArea');
    
     S.Unique_Z_ID_S = join(strcat([string(S.("CodedFile#")),...
         string(S.("Cell#")),string(S.Shape_Z)]),"_");
     S = [S(:,1:5) S(:,end) S(:,6:end-1)];
     warning('on','all')   
 return;